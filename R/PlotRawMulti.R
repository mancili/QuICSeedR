...
#' }
#' @export
PlotRawMulti = function(raw, samples, legend_position = "topleft", ylim = NULL, xlim = NULL) {
  ...
  if (length(sel) > 0) {
    ...
    plot(x = time[not_na_indices], y = data_copy[not_na_indices, sel[1]],
         type = "l", col = 1, ylim = ylim, xlim = xlim,
         xlab = "Time (h)", ylab = "Fluorescence")
    for (i in 2:length(sel)) {
      not_na_indices <- !is.na(data_copy[, sel[i]])
      lines(x = time[not_na_indices], y = data_copy[not_na_indices, sel[i]],
            type = "l", col = 1)
    }
  }
  ...
  legend(legend_position, legend = samples, col = seq_len(length(samples)), lty = 1, cex = 0.8)
}
